var Message = function(options){
	...
	/*---------------------------------properties-------------------------------*/
	_this.messagerUrl = 'http://www.smogon.com/forums/conversations/add';
	_this.postUrl = 'http://www.smogon.com/forums/conversations/insert';
	_this.leaveUrl = 'http://www.smogon.com/forums/conversations/{conversationId}/leave';

	_this.to = '';
	_this.title = '';
	_this.message = '';
	_this.allowInvite = true;
	_this.noReply = false;
	_this.token = '';
	...
	var removeFromHistory = function(callback) {
		if(_this.conversationId !== -1) {
			$.ajax({
				...
				success:function(){
					defined(callback)({ success:true, info:'Data sent.' });
				},
				error:function(){
					defined(callback)({ success:true, info:'Data sent.' });
				}
	...
	_this.send = function(callback) {
		if(!prepared) {
			...
			return defined(callback)({ success:false, info:'Data is already being sent.' });
		} else if(sent) {
			return defined(callback)({ success:false, info:'Data has already been sent.' });
		}
		...
		$.ajax({
			...
			error:function() { 
				defined(callback)({ success:false, info:'There was a problem sending the data. This is likely due to a script error.' });
			},
			complete:function(){
				isSending = false;
			}
